...
//!
//! # 性能注意
//! 当前每次调用都立即 `emit("task_log", ...)`，对于每文件都打日志的热路径
//! （dedup / cleanup / scan / rename）会在大目录下产生大量 IPC 事件。前端
//! `runtime` store 已经做了 150ms 节流缓冲来吸收抖动；进一步的"后端批量
//! 发送"需要新增 `task_log_batch` 事件并同步前端监听器，属于跨端契约变更，
//! 故暂未实现。热路径请尽量只在 chunk / 阶段边界打日志，避免逐条发送。
