...
- **Mod 工具**：针对 Illusion 系列 `.zipmod` 的六类操作——
  ...
  - **归类**：按文件名首个 `[...]` 建子目录归类。
  - **重复 MOD 检查**：按 `guid + author + version` 分组找重复，默认每组保留最新；删除移动到 `.fileflow-del-*` 备份路径并可撤回。
  - **不同版本 MOD 检查**：按 `guid + author` 分组找多个 `version`，默认保留最高版本；同样备份+可撤回。
  - **移除版本限制（modify）**：就地重写 zip，从 manifest.xml 去掉指定 `<game>KEYWORD</game>` 标签，原文件备份。
...
后缀修改、空文件夹清理、Mod 重命名 / 归类 / 重复删除 / 旧版本删除 / 修改版本限制，本质都是："产生一批 `(old_path, new_path)` → 批量执行 → 写入可回滚记录"。FileFlow 把这一模式抽成三层，**任何新增此类业务都必须走这套抽象，严禁复制 suffix / mod_tools 当模板**。
...
- **外部配置**：数据库路径存 `<app_data_dir>/fileflow_config.json`（鸡生蛋问题——无法把 db 路径存进 db 本身）。后端 `external_config::resolve_db_path` 已做"是目录就追加 fileflow.db"的兜底，前端选目录后直接传即可。
